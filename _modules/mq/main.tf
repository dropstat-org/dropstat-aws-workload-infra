# ============================================================
# _modules/mq
# Amazon MQ (ActiveMQ) — VPC and subnets discovered via tm-aws-account-data
# ============================================================
...
module "sg_mq" {
  ...
  ingress_with_source_security_group_id = flatten([
    for sg_id in var.allowed_security_group_ids : [
      { from_port = 1883, to_port = 1883, protocol = "tcp", source_security_group_id = sg_id, description = "MQTT" },
      { from_port = 5671, to_port = 5671, protocol = "tcp", source_security_group_id = sg_id, description = "AMQPS" },
      { from_port = 8162, to_port = 8162, protocol = "tcp", source_security_group_id = sg_id, description = "ActiveMQ Web Console" },
    ]
  ])
